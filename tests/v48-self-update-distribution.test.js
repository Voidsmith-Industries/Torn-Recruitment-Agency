...
const STABLE_URL = 'https://voidsmithindustries.com/torn/install/recruitment-agency.user.js';

test('v4.8 migration release points future userscript updates at the Voidsmith website', () => {
  assert.ok(userscript.includes('// @downloadURL  ' + STABLE_URL));
  assert.ok(userscript.includes('// @updateURL    ' + STABLE_URL));
});

test('v4.8 release metadata keeps a single website update authority', () => {
